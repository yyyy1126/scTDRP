...
- ✅ **Figure 1 方法示意图 + AEL 实证面板**：已生成，脚本为 `make_figure1.py`，输出见 `figures_figure1/`。
- ⚠️ **Figure 2-4 生成脚本待补齐**：正文已有 Figure 2（AEL intra-tumor）、Figure 3（B-ALL）、Figure 4/S1 的描述，但缺少可复现脚本。建议补充 `make_figure2.py`、`make_figure3.py`、`make_figure4.py`。

### 投稿前下一步行动（Action Plan）

按优先级排序，供冲刺投稿时参考：

#### P0：必须做（不做会被拒或大幅返修）

1. **整合 Figure 1 / Figure 2 并统一编号**
   - 当前 Figure 1 已覆盖原 Figure 2 的 AEL cost map / TDI / repair / module 内容。
   - 建议：Figure 1 保留为方法总览 + AEL proof-of-concept；删除原 Figure 2；原 Figure 3 → Figure 2，原 Figure 4 → Figure 3。
   - 同步批量替换正文中的 Figure 引用并更新 Figure Legends。

2. **补齐 Figure 2-4 的生成脚本**
   - 目前已有 `make_figure1.py`。
   - 待补充：`make_figure2.py`（AEL intra-tumor）、`make_figure3.py`（B-ALL）、`make_figure4.py`（GBM + OT 方法比较）。
   - NC 对代码可复现性要求很高，每个主图都应有独立脚本。

3. **补充 gene-level WOT / Moscot 比较**
   - 当前比较停留在 PCA space（stage assignment 一致率 + repair direction cosine）。
   - 建议对 Moscot transport matrix 计算 gene-level repair delta，比较 scTDRP / WOT / Moscot 的 top repair targets 重叠率（Jaccard）和 repair score 相关性（Spearman）。

4. **全文数字一致性检查**
   - 统一同一数据集的细胞数、TDI、module scores、p 值来源。
   - 当前 Figure 1 用 n=1,943，正文 Figure 2 用 n=2,134，需明确过滤条件差异。

#### P1：强烈建议做（提升命中率）

5. **补充数学稳定性 / 模拟的 Methods 细节**
   - 明确模拟次数、样本量梯度、收敛判据。
   - 加一张 supplementary figure 展示误差随样本量下降。

6. **整理 Supplementary Information 结构**
   - Supplementary Methods：metacell、GBM 验证、OT 比较、数学模拟。
   - Supplementary Figures：MDS-5q、GBM、gene-level OT 比较、数学稳定性。
   - Supplementary Tables：模块基因列表、top repair targets。

7. **生成投稿级高分辨率图片**
   - Figure 1-4 导出为 TIFF/EPS，300 dpi（组合图建议 600 dpi）。
   - 确认字体嵌入、颜色模式正确。

#### P2：可选但加分

8. **撰写 Cover Letter**
   - 三段式：问题重要性、scTDRP 核心创新、为什么适合 NC（跨疾病 + 与 WOT/Moscot 直接对照）。

9. **外层仓库与 Zenodo 归档整理**
   - 确认 Zenodo `10.5281/zenodo.15640823` 包含最新代码/数据。
   - 处理 `5.external/scTDRP` 与外层 Gitee 仓库的关系（subtree / submodule / 明确分离）。
